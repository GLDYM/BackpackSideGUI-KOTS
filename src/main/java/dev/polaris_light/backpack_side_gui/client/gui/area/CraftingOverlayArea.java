...
public final class CraftingOverlayArea extends IOverlayArea {
        ...
        if (itemDragging)
            for (int i : dragSlots) {
                ...
                if (base.isEmpty() || ItemStack.isSameItemSameComponents(base, dragCarried)) {
                    slots[i].renderPreview(graphics, minecraft, x + Layout.INPUT_X + (i % 3) * Layout.SLOT_SIZE,
                            y + Layout.INPUT_Y + (i / 3) * Layout.SLOT_SIZE,
                            (base.isEmpty() ? dragCarried : base).copyWithCount(base.getCount() + amount));
                    slots[i].renderDragHighlight(graphics, x + Layout.INPUT_X + (i % 3) * Layout.SLOT_SIZE,
                            y + Layout.INPUT_Y + (i / 3) * Layout.SLOT_SIZE);
                }
            }
    }
    ...
    public boolean mouseDragged(ScreenEvent.MouseDragged.Pre event) {
        if (!itemDragging)
            return false;
        int col = (int) ((event.getMouseX() - x - Layout.INPUT_X) / Layout.SLOT_SIZE);
        int row = (int) ((event.getMouseY() - y - Layout.INPUT_Y) / Layout.SLOT_SIZE);
