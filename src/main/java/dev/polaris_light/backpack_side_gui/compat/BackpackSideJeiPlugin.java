...
public final class BackpackSideJeiPlugin implements IModPlugin {
    ...
    public void onRuntimeAvailable(IJeiRuntime runtime) {
        JeiReflectionCompat.setRuntime(runtime);
    }

    @Override
    public void onRuntimeUnavailable() {
        JeiReflectionCompat.clearRuntime();
    }
    ...
    public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration) {
        registration.addUniversalRecipeTransferHandler(new IUniversalRecipeTransferHandler<CraftingMenu>() {
            ...
            public java.util.Optional<net.minecraft.world.inventory.MenuType<CraftingMenu>> getMenuType() {
                return java.util.Optional.empty();
            }

            public IRecipeTransferError transferRecipe(CraftingMenu menu, Object recipe, IRecipeSlotsView slots,
                    Player player, boolean maxTransfer, boolean doTransfer) {
                if (!SideBackpackClient.isCraftingUtilityVisible())
                    return null;
                if (doTransfer) {
                    List<List<ItemStack>> groups = new ArrayList<>();
                    slots.getSlotViews(mezz.jei.api.recipe.RecipeIngredientRole.INPUT).stream().limit(9)
                            .forEach(slot -> groups.add(slot.getItemStacks().map(ItemStack::copy).toList()));
                    while (groups.size() < 9)
                        groups.add(List.of());
